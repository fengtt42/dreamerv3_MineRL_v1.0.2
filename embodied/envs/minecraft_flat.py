...
class MinecraftBase(embodied.Env):
  ...
  def __init__(
    ...
    self.plank_types = [
        'acacia_planks', 'birch_planks', 'dark_oak_planks', 'jungle_planks',
        'oak_planks', 'spruce_planks', 'crimson_planks', 'warped_planks'
    ]
    # Observations
    self._inv_keys = [
        k for k in self._env.obs_space if k.startswith('inventory/')
        if k not in [f'inventory/{log}' for log in self.log_types]
          and k not in [f'inventory/{plank}' for plank in self.plank_types]]
    self._inv_keys.append('inventory/log')
    self._inv_keys.append('inventory/planks')
